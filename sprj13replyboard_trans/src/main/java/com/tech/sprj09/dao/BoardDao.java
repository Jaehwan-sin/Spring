...
//							String sql = "select bid,bname,btitle,bcontent,bdate,bhit,bgroup,bstep,bindent from replyboard"; // 17 데이터 불러오는 sql문
							String sql = "select bid,bname,btitle,bcontent,bdate,bhit,bgroup,bstep,bindent from replyboard order by bgroup desc,bstep asc"; // 64 그룹별로 보이게 하는 sql문
...
//				replyShape(bgroup,bstep);
				...
				try {
					con = dataSource.getConnection();
					// 68 커넥션 생성후 replyShape에 전달 (트랜젝션 처리)
					
					// 79 con의 Autocommit 상태를 false로 변경 
					...
					rn2 = pstmt.executeUpdate(); // 77
					System.out.println("rn2 : "+rn2);
//					현재 con의 기본 commit 상태 확인 78
			...
			/* 66 step DB 작업 */
			private int replyShape(String bgroup, String bstep, Connection con) {// 69 reply에서 보낸 con을 받음 / 73 int로 받고 리턴생성
				
//				Connection con = null; 70 전달 받아서 필요없음.
				PreparedStatement pstmt = null;
				int rn1 = 0; // 74
...
//					con = dataSource.getConnection(); 전달 받아서 필요없음
					...
					try {
						// 사용한 자원 회수
						if (pstmt!=null) pstmt.close();
//						if (con!=null) con.close(); 71 여기서 close하면 reply에서 연결이 안되서 닫으면 안된다.
					} catch (Exception e2) {
						// TODO: handle exception
					}
				}
