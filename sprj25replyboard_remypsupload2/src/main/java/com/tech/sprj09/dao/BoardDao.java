...
//			/* 10 생성자 만들기*/ 다른 방법으로 DB접속하기 위해 주석처리
//			public BoardDao() {
...
//							String sql = "select bid,bname,btitle,bcontent,bdate,bhit,bgroup,bstep,bindent from replyboard"; // 17 데이터 불러오는 sql문
							...
							while (rs.next()) {
								...
								// 20 생성자를 통해 BoardDto에 담아주기
									BoardDto dto = new BoardDto(bid, bname, btitle, bcontent, bdate, bhit, bgroup, bstep, bindent,null);
								// 21 리스트에 담기
									dtos.add(dto);
							}
			...
			public BoardDto contentView(String sbid) {
				...
				try {
					...
					if (rs.next()) {
						...
						dto = new BoardDto(bid, bname, btitle, bcontent, bdate, bhit, bgroup, bstep, bindent,null);
					}
				} catch (SQLException e) {
					// TODO Auto-generated catch block 
					e.printStackTrace();
